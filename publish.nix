    ...
    { buildInputs = packages;
		shellHook = ''
			...
			if [ "$BACKEND" == "ECR" ]; then
				...
				! $(aws ecr describe-repositories --region ap-southeast-2 | jq '.repositories[].repositoryName' | grep "$REPO_NAME" -q) && aws ecr create-repository --region ap-southeast-2 --repository-name $REPO_NAME
			fi
